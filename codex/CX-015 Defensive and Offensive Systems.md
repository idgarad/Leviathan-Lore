## Weapon System Waste Heat Generation

Each weapon system generates waste heat when fired. This heat must be managed by the ship's thermal systems and is a critical factor in sustained combat. Waste heat values are subject to future balancing passes.

- **Kinetic Weapons (Railguns, Cannons):**
    - Generate a fixed amount of waste heat per shot (in MJ), determined by the weapon's design. Example: 2 MJ per shot.

- **Plasma Weapons:**
    - Generate waste heat equal to 5% of their MJ output per shot.
    - $H_{waste,plasma} = 0.05 \times E_{shot}$

- **Beam Weapons:**
    - Generate waste heat equal to 2% of their MJ output per tick (continuous while firing).
    - $H_{waste,beam} = 0.02 \times E_{tick}$

+- **Missile Launchers:**
    - Waste heat generated by missile launchers is negligible and is sub-MJ per VOLLEY due to gimbal or turret mounting. Assume 0.5MJ

> **Note:** These values are initial assumptions and should be revisited during balancing and playtesting.
